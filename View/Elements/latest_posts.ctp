...
<div class="avatar">
<img src="<?php echo $post['CreatedBy']['avatar_url'] ?>" alt="Avatar de <?= $post['CreatedBy']['username'] ?>" />
</div>
...
<td>
<div class="post-date"><?php echo $this->Html->link($this->Time->timeAgoInWords($post['created']), array('controller' => 'topics', 'action' => 'view', $post['topic'], '#' => 'p' . $post['id'])) ?> <?php if ($post['edited']) : ?>(édité par <?= $post['EditedBy']['username'] ?>, <?= $this->Time->timeAgoInWords($post['edited']) ?>)<?php endif ?></div>
<div class="post-body">
